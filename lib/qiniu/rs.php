...
class Qiniu_RS_PutPolicy
{
	...
	public $CallbackFetchKey;	//0 is off, 1 is on,default 0
	public $ReturnUrl;
	public $ReturnBody;
	public $AsyncOps;
	public $EndUser;
	...
	public function Token($mac) // => $token
	{
		...
		if (!empty($this->CallbackBodyType)) {
			$policy['callbackBodyType'] = $this->CallbackBodyType;
		}
		if (!empty($this->CallbackFetchKey)) {
			$policy['callbackFetchKey']=$this->CallbackFetchKey;
		}
